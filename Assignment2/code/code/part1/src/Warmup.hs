...
instance Monad RWSP where
  ...
  m >>=  f  = RWSP (\r s -> let (r0, w0, s0) = runRWSP m r s
                                (r1, w1, s1) = runRWSP (f r0) r s0
                            in  (r1, w0 `mappend` w1, s1))
...
instance Monad RWSE where
  ...
  m >>=  f = RWSE (\r s -> case runRWSE m r s of
                                (Left e) -> Left e
                                Right (r0, w0, s0) -> case runRWSE (f r0) r s0 of
                                                            (Left e) -> Left  e
                                                            Right (r1, w1, s1) -> Right (r1, w0 `mappend` w1, s1))
